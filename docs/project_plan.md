...
6. Baselines
   ...
   - PPO: use TRL PPO where feasible, implement a toy comparison, or document compute
     limits and compare against supervised/evaluation baselines.
...
The repository is now organized around a two-layer experiment:

1. Local 8GB-oriented experiments:
   - baseline inference on a fixed GSM8K eval subset
   - DPO from offline synthetic preference pairs
   - GRPO smoke/lite with small group size and short completions
2. Optional larger-GPU experiments:
   - PPO with the same base model
   - larger GRPO group sizes or more training steps
   - multi-seed stability runs

The immediate engineering target is to make baseline, DPO, and GRPO runnable through
Makefile targets and shell scripts with `uv` as the preferred launcher while still
supporting standard pip/venv workflows via `PYTHON=python` or `PYTHON_CMD=python`.
