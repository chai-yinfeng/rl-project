...
def train_ppo_trl(config: dict[str, Any]) -> dict[str, Any]:
    ...
    summary = {
        ...
        "effective_batch_size": int(config.get("per_device_train_batch_size", 1))
        * int(config.get("gradient_accumulation_steps", 1)),
        ...
        "kl_coef": float(config.get("kl_coef", config.get("kl_beta", 0.0))),
        "num_ppo_epochs": int(config.get("num_ppo_epochs", 0) or 0),
        "num_mini_batches": int(config.get("num_mini_batches", 0) or 0),
        "response_length": int(config.get("response_length", config.get("max_new_tokens", 0)) or 0),
        "use_peft": bool(config.get("use_peft", True)),
...
def train_ppo_style(config: dict[str, Any]) -> dict[str, Any]:
    ...
    summary = {
        ...
        "kl_coef": kl_beta,
        "response_length": int(config.get("max_new_tokens", 0) or 0),
        "use_peft": bool(config.get("use_peft", True)),
        "elapsed_seconds": time.time() - start_time,
        **cuda_memory_summary(),
    }
